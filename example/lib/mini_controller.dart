...
class MiniController extends ValueNotifier<VideoPlayerValue> {
  ...
  Future<void> initialize() async {
    ...
    void eventListener(VideoEvent event) {
      log("Event listener type ${event.eventType}");
      log("Event listener duration ${event.duration}");
      log("Event listener buffered ${event.buffered}");
      log("Event listener rotationCorrection ${event.rotationCorrection}");
      log("Event listener size ${event.size}");
  ...
  Future<void> setDataSource(DataSource dataSourceDescription) async {
    ...
    log("Creating completer is not complete ");
    if (_creatingCompleter?.isCompleted == false)
      await _creatingCompleter?.future;
    log("seting up data source");
    final Completer<void> initializingCompleter = Completer<void>();
   var message = await _platform.setDataSource(_textureId, dataSourceDescription);
    return initializingCompleter.future;
  }
